...
function func_microbasinMap() {
	var nombreComunidad = '';
	var x_mapa_comunidad = 0;
	var y_mapa_comunidad = 0;
	// MOSTRAR INFORMACIÓN DE REGION CON EFECTO MOUSEENTER PARA MAPA
	var eventComunityCircles = document.querySelectorAll("#microcuenca_comunidad_circulos_hover circle");
	eventComunityCircles.forEach(function(eventComunityCircle){
		eventComunityCircle.addEventListener("mouseenter", function(){
			if(window.outerWidth > 1024){
				...
				document.querySelector("#mapa_microcuenca_comunidad_container").style.left = x_mapa_comunidad+"px";
				document.querySelector("#mapa_microcuenca_comunidad_container").style.display = "block";
			}
		});

		eventComunityCircle.addEventListener("mouseleave", function(){
			if(window.outerWidth > 1024){
		...
		eventComunityCircle.addEventListener("click", function(){
	...
	function selectVillage(nombreComunidad){
		document.querySelector('#microcuenca_comunidad_circulos #cir_' + nombreComunidad).classList.add("selectComunidadCirculo");
		document.querySelector('#microcuenca_comunidad_numeros #nro_'+ nombreComunidad).classList.add("selectComunidadNumero");
		document.querySelector('#microcuenca_comunidad_nombres #nom_'+ nombreComunidad).style.display = "block";
	}

	function restoreMap(){
		var comunityCircles = document.querySelectorAll('#microcuenca_comunidad_circulos circle');
		comunityCircles.forEach(function(comunityCircle){
			comunityCircle.classList.remove("selectComunidadCirculo");
		});
		var comunityNumbers = document.querySelectorAll('#microcuenca_comunidad_numeros text');
		comunityNumbers.forEach(function(comunityNumber){
			comunityNumber.classList.remove("selectComunidadNumero");
		});
		var comunityNames = document.querySelectorAll('#microcuenca_comunidad_nombres text');
		comunityNames.forEach(function(comunityName){
			comunityName.style.display = "none";
		});
		document.querySelector("#mapa_microcuenca_comunidad_container").style.display = "none";
	}
}

function func_scopeMap() {
	var nombreRegion = '';
	var x = 0;
	var y = 0;
	var tituloRegion = '';
	
	//recuperar ambito de trabajo a un array
	
	listarNroFamiliasRegion();
	var regionList = [];
	
	function listarNroFamiliasRegion(){
		fetch('/manage/retrieve/regions')
		.then(response => response.json())
		.then(result => {
			regionList = result;
		})
	...
	function buscarAmbitoEnArreglo(region){
		var nroFam = 0;
		for(var i = 0; i < regionList.length; i++){
			if(regionList[i]["name"] == region){
				nroFam = regionList[i]["scope"];
				break
			}
		}
		return nroFam;
	...
	// MOSTRAR INFORMACIÓN DE REGION CON EFECTO MOUSEENTER PARA MAPA
	var eventRegionNames = document.querySelectorAll("#mapa_peru_nombres text");
	eventRegionNames.forEach(function(eventRegionName){
		eventRegionName.addEventListener("mouseenter", function(){
			if(window.outerWidth > 1024){
				...
				// crear información
				nombreRegion = eventRegionName.getAttribute("id");
				tituloRegion = eventRegionName.innerHTML;
				var nroFamilias = buscarAmbitoEnArreglo(tituloRegion);
				// mostrar información
				document.querySelector("#area_" + nombreRegion).classList.add("select-background");
				document.querySelector("#information_region").style.top = y+"px";
				document.querySelector("#information_region").style.left = x+"px";
				document.querySelector("#information_region").style.display = "block";
				document.querySelector('#information_region').textContent = nroFamilias + " familias";
			}
		});

		eventRegionName.addEventListener("mouseleave", function(){
			if(window.outerWidth > 1024){
		...
		eventRegionName.addEventListener("click", function(){
			if(window.outerWidth < 1025){
				// set position to selected region information
				var point = eventRegionName.getBoundingClientRect();
				let mapa_alcance_territorial = document.querySelector('#mapa_alcance_territorial').getBoundingClientRect();
	...
	function restoreMap(){
		var polygons = document.querySelectorAll("#regiones_con_piloto polygon");
		polygons.forEach(function(polygon){
			polygon.classList.remove("select-background");
		});
		var paths = document.querySelectorAll("#regiones_con_piloto path");
		paths.forEach(function(path){
			path.classList.remove("select-background");
		});
		var polygons = document.querySelectorAll("#regiones_sin_piloto polygon");
		polygons.forEach(function(polygon){
			polygon.classList.remove("select-background");
		});
		var paths = document.querySelectorAll("#regiones_sin_piloto path");
		paths.forEach(function(path){
			path.classList.remove("select-background");
		});
		document.querySelector("#information_region").style.display = "none";
	}

	// SWITCH BETWEEN LANGUAGES
	// var es_lang = document.getElementById("es_lang");
	// es_lang.addEventListener('click', () => {
	// 	// get the current page in spanish
	// 	var page_name = window.location.pathname.slice(1);
	// 	fetch('retrieve/pagename/es/' + page_name)
	// 	.then(response => response.json())
	// 	.then(result => {
	...
	// 	// get its corresponding page name in english
